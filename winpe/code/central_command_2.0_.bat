...
if "%choice%"=="99" goto let_it_diag
if "%choice%"=="999" goto let_it_run
...
echo.
set /p choice=Type option:
if "%choice%"=="01" goto network_reset_start
if "%choice%"=="02" goto winsec_reset_start
if "%choice%"=="03" goto start_menu_reset
if "%choice%"=="04" goto usoclient_reset
if "%choice%"=="9" goto main_menu
if "%choice%"=="0" goto exit
goto error_program
pause>null
...
:network_reset_start
echo %subdivider1%
echo resetting network adapters, connections, and firewall: & ipconfig /release & ipconfig /flushdns & ipconfig /renew & netsh winsock reset & netsh interface ipv4 reset & netsh interface ipv6 reset & netsh winsock reset catalog & netsh int ipv4 reset reset.log & netsh int ipv6 reset reset.log & netsh int ip reset & ren c:\windows\softwaredistribution softwaredistribution.old & ren c:\windows\system32\catroot2 catroot2.old & echo resetting windows firewall & netsh advfirewall reset & echo restoring local firewall defaults: & powershell -command "(new-object -comobject hnetcfg.fwpolicy2).restorelocalfirewalldefaults()" & echo windows firewall restored to default settings
echo %subdivider1%
::pause>null
goto reset_submenu
:winsec_reset_start
...
start cmd.exe /c (echo initializing serial number in wmic & dism /online /add-capability /capabilityname:wmic~~~~)
goto :eof
:bcdedit_start
echo %subdivider1%
call :bcdedit
...
if "%choice%"=="04" goto winsec_update_start
if "%choice%"=="05" goto winsec_reset_start
...
:: =============================
:new_setup_nf
echo starting new setup suite: & echo %subdivider2% & call :restore_enable & call :shadow_size_5 & call :10 & call :quick_save_exit & call :1440 & call :gs_support & start "" %driveletter%\win\device_tools\user_accounts.lnk & call :net_reset_suite & call :timeout & call :revo & call :timeout & call :updates_win_nf & call :winsec_suite & call :timeout & call :winget_pchealth & call :winget_update & start ms-settings:storagesense & call :timeout & call :powercfg_ds & call :bde_off_c & echo blue screen section: & call :bsod_on & echo numlock section & call :numlock_on & call :s_mode_reg_0 & echo %subdivider2% & goto :eof
